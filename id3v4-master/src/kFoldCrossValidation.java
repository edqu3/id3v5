...
public class kFoldCrossValidation {
    ...
    public static void crossValidate(AttributeCollection ac, int k_folds){
         ...
         List<List<Attribute[]>> partitions = partition(ac.collection, k_folds);
        ...
        for (int testIndex = 0; testIndex < partitions.size(); testIndex++) {
            ...
            DTNode dTree = new DTNode();
            Algorithm.id3(dTree, newTrainingSet, "TARGET");
            // bypass first empty node to get to true root node.
            dTree = dTree.branches.get(0);
            // cross validate
            BigDecimal a = validate(newTestingSet, dTree, "TARGET");
